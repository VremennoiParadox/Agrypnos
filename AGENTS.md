...
**Unlocked (Core landed; UI still shipping):** Discord inbound two-way — user’s own Discord bot. Not claimed shipped. Not Mac-proven until UI + optical.
...
- **UI (still shipping):** Notif Discord inbound chrome mirrors Telegram (inbound on/off + short command/help list + bot token + channel id). Keep the webhook paste block outbound-only — never imply the webhook receives commands.
- Soft Mac prove after UI: inbound on → `/` menu shows the four commands; `/help` honesty + asleep note + lid-gated `/disarm`; cmds while asleep → on wake missed-while-asleep, no auto-apply; lid open `/disarm` → watch off, Mac stays awake; confirmed lid-closed `/disarm` → watch off + Mac sleeps. No live-asleep reply. Do not claim shipped or Mac-proven until that check. Core is landed; UI still shipping.
...
Good: “If the bot isn’t replying, the Mac is likely asleep / Agrypnos isn’t receiving updates.” (Discord `/help` — after it ships)
