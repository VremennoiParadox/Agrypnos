...
- **Notif** — V2 slice, popover-only. Outbound idle-after-wait POST is **landed**. Telegram inbound two-way is **landed** (Mac-proven). Telegram inbound polish is **landed** (Mac-proven): BotFather slash menu `/arm` `/disarm` `/status` `/help`, wake-miss drain, lid-gated Telegram `/disarm`. Facts-richer Telegram `/status` dump + `/help` mirror is **landed** (Mac-proven, user optical 2026-09-24): dump live WatchEngine state Agrypnos already knows. Live laptop battery on `/status` is **unlocked** (Core + UI after Review CLEAR + Boss merge of this bar) — extend that dump; reply copy only, not a Notif card. Discord inbound is **unlocked** (Core + UI after Review CLEAR + Boss merge of this bar) — user’s own Discord bot token + channel id; webhook stays outbound-only; same WatchEngine path as Telegram. Not claimed shipped. Do not grow past that. Enriched-status B (task text / finish ETA) is **scrapped** — will not implement; Review blocks any PR that adds it.
...
**Unlocked (Core + Agrypnos UI after Review CLEAR + Boss merge of this bar):** Discord inbound two-way — user’s own Discord bot. Not claimed shipped. Not Mac-proven until soft prove.
...
- **UI (after Core+UI):** Notif Discord inbound chrome mirrors Telegram (inbound on/off + short command/help list + bot token + channel id). Keep the webhook paste block outbound-only — never imply the webhook receives commands.
- Soft Mac prove after implement: inbound on → `/` menu shows the four commands; `/help` honesty + asleep note + lid-gated `/disarm`; cmds while asleep → on wake missed-while-asleep, no auto-apply; lid open `/disarm` → watch off, Mac stays awake; confirmed lid-closed `/disarm` → watch off + Mac sleeps. No live-asleep reply. Do not claim shipped or Mac-proven until that check.
...
Soft Mac prove after implement: A = floor+ramp, no surprise display-sleep claim; B = confirmed lid-close panel actually sleeps, Keep the watch still on, Mac not put to sleep by B; switch A↔B exclusive; open-lid disarm/use does not leave the panel wrongly asleep; Power open shows picker labels + a caption that states A vs B; B hides ramp chrome and help says why. Ban watt claims, Wi-Fi/BT kill, inventing sleep while the lid is open. Not claimed shipped. Enriched-status B (task text / finish ETA) is **scrapped**.

**Landed (Mac-proven):** facts-richer Telegram `/status` dump + `/help` mirror — dump live WatchEngine state Agrypnos already knows (Keep the watch on/off; How long mode named, no fake countdown on remembered-only; lid open or unconfirmed vs confirmed closed; Agents selected / busy-seen-this-arm / settle-wait when relevant; last-end honesty if any; safety prefs that affect the watch). Reply copy only — not a new Notif popover control. `/help` says (short) that `/status` returns these watch facts; keep asleep note + lid-gated `/disarm`. User optical **passed** 2026-09-24. Ban invent ETA, think-detection, countdown on remembered-only How long. Live battery is **unlocked above**, not claimed shipped. Enriched-status B (task text / finish ETA) is **scrapped**.
...
- Soft verify (Power A/B): after Core+UI, rebuild. A = confirmed lid-close floor + ramp, no surprise display-sleep claim. B = confirmed lid-close **panel** actually sleeps (`displaysleepnow` or equivalent); Keep the watch still on; Mac not put to sleep by B; agents still running; ramp chrome hidden. Switch A↔B exclusive (never floor + `displaysleepnow` together). Open-lid disarm/use does not leave the panel wrongly asleep. Unconfirmed close applies neither. Power open shows picker labels + a caption that states A vs B; B help says why ramp chrome is hidden. Copy must not say B made the Mac asleep, ended Keep the watch, or ended agent work. Do not claim Mac-proven until that check.
- Soft verify (Discord inbound): after Core+UI, rebuild. Inbound on → Discord `/` menu shows `/arm` `/disarm` `/status` `/help` (or Discord-native slash mapped to those); `/help` honesty + asleep note + lid-gated `/disarm`; cmds while asleep → on wake “missed while asleep”, no auto-apply; lid open `/disarm` → watch off, Mac stays awake; confirmed lid-closed `/disarm` → watch off + Mac sleeps. Same WatchEngine path as Telegram — no second arm/disarm stack. Webhook still outbound-only. No live-asleep reply. Do not claim Mac-proven until that check.
...
Good: “Armed. Waiting for lid close — then the panel sleeps + keyboard backlight off. Keep the watch still holds the Mac awake.” (Power B — after it ships)
Good: “Brightness floor. Panel stays on (dimmed).” (Power A)
Good: “Display asleep.” (Power B — after it ships)
Good: “Panel sleeps on confirmed lid close. Keep the watch still holds the Mac awake.” (Power B — after it ships)
...
Good: “Battery 62% · discharging” / “Battery 62% · on AC” (Telegram or Discord `/status` live battery after it ships — omit if unknown)
...
Good: “Dim panel” / “Sleep panel” (Power A/B picker — after it ships; or clearer plain equivalents)
Good: “Dim panel: brightness floor on confirmed lid close. Panel stays on, dimmed. Sleep panel: the panel sleeps. Keep the watch still holds the Mac awake — agents keep running.” (Power caption under the picker — after it ships)
Good: “Brightness return is hidden because it only applies when restoring from the floor path.” (Power help while B — after it ships)
