...
- **Notif** — V2 slice, popover-only. Outbound idle-after-wait POST is **landed**. Telegram inbound two-way is **landed** (Mac-proven). Telegram inbound polish is **landed** (Mac-proven): BotFather slash menu `/arm` `/disarm` `/status` `/help`, wake-miss drain, lid-gated Telegram `/disarm`. Facts-richer Telegram `/status` is **unlocked** (Core + UI after Review CLEAR + Boss merge of this bar): dump live WatchEngine state Agrypnos already knows. Do not grow past that. Do **not** unlock parked rich status (task text / finish ETA).
  ...
  - **Telegram `/status` facts dump (unlocked — Core + UI after Review CLEAR + Boss merge of this bar).** Dump live WatchEngine state Agrypnos already knows. Facts only. Omit lines that don’t apply or aren’t known. Not parked rich status (task text / finish ETA) — that stays idea-only. Ban invent ETA, “still thinking,” “job finished,” think-detection, and a countdown on remembered-only How long (`∞` / `1h` / `3h` / custom do not auto-off; Agents settle is not that clock). `/status` reply must be able to include:
    ...
    6. Safety prefs that affect the watch: low-battery threshold; thermal auto-off on/off. LPM copy honesty as already specified (no ended-copy while forced watch still holds) — omit an LPM line if that state is unknown; do not invent a sensor.
    Soft Mac prove after implement (rebuild, `/status` lines match live state). Do not claim Mac-proven until that check.
  ...
  - **Self-serve docs (required):**
    - README: step-by-step Discord (Server Settings → Integrations → Webhooks → New Webhook → copy URL) and Telegram (BotFather `/newbot` → token; message the bot; get chat id via `getUpdates` or a clear equivalent). What to paste where in Agrypnos. How to test outbound (arm Agents, produce a local busy signal, wait idle, expect one POST, then Keep the watch off). How to enable inbound (same BotFather bot; chat with the bot; inbound on in Notif; `/arm` `/disarm` `/status` `/help`). BotFather menu matches those four via `setMyCommands` or equivalent; `/help` asleep note + confirmed lid-gated `/disarm`; after the `/status` dump ships, `/help` also says `/status` returns these watch facts (short); wake-miss drain without applying (not leftover setup/quit drain). How to turn off / clear secrets. Discord inbound is not this slice. Landed `/status` is Keep the watch on/off; when on, How long; do not describe the facts dump as already shipped.
    - In-app Notif help must point at the same steps (short in the popover; full detail in README, linked or paraphrased).
  - **Out of this slice (idea-only):** rich status (task text / finish ETA) stays parked. Facts-richer Telegram `/status` (live WatchEngine dump) is **unlocked above**, not this parked item. Discord inbound (webhook stays outbound; Discord bot-token inbound). Notification Center as the Notif path. Do not invent them.
...
**Unlocked (Core + Agrypnos UI after Review CLEAR + Boss merge of this bar):** facts-richer Telegram `/status` — dump live WatchEngine state Agrypnos already knows (Keep the watch on/off; How long mode named, no fake countdown on remembered-only; lid open or unconfirmed vs confirmed closed; Agents selected / busy-seen-this-arm / settle-wait when relevant; last-end honesty if any; safety prefs that affect the watch). Reply copy only — not a new Notif popover control. After the dump ships, `/help` says (short) that `/status` returns these watch facts; keep asleep note + lid-gated `/disarm`. Notif inbound polish stays landed/Mac-proven. Soft Mac prove after implement (rebuild, `/status` lines match live state). Ban invent ETA. Parked rich status (task text / finish ETA) stays idea-only. Discord inbound / panel sleep stay parked.
...
**Landed (Mac-proven):** Open-lid brightness — no surprise dim to floor after disengage with the lid open (user optical, 2026-09-24). Floor write only when armed + lid confirmed closed; nil capture skips the write.
...
Rich status (task text / finish ETA) stays idea-only — do not unlock. Facts-richer Telegram `/status` (live WatchEngine dump) is **not** this — that dump is unlocked separately above.
...
- Soft verify (facts-richer Telegram `/status`): after implement, rebuild; `/status` lines match live WatchEngine state (armed/disarmed, How long mode, lid open vs confirmed closed, Agents facts when relevant, last-end if any, safety prefs). Omit unknown lines. No ETA, no think-detection, no countdown on remembered-only How long. `/help` names that `/status` returns these watch facts and still has the asleep note + lid-gated `/disarm`. Do not claim Mac-proven until that check.
...
Good: “Keep the watch is on. How long is Agents. Lid open.” (Telegram `/status` after the dump ships — facts only; omit lines that don’t apply)
Good: “/status returns Keep the watch, How long, lid, Agents facts when relevant, last watch end, and safety prefs.” (`/help` after the dump ships — short)
...
Bad: inventing a finish ETA or task text in `/status` (parked rich status — do not unlock).
