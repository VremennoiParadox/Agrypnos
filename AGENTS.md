...
- **Armed ≠ black screen.** Toggling Keep the watch must **not** call `displaysleepnow`, blank the panel, or kill the keyboard backlight while the lid is open. Do not claim “we force display asleep”, “screen off”, or “dim ≠ asleep / real display sleep” for the **toggle** or for **Power A**. Landed lid-close honesty for **Power A** (default, Mac-proven) is **brightness floor + keyboard off**. **Power B** (landed, Mac-proven) is real **panel/display** sleep on **confirmed** lid-close only (`displaysleepnow`) — that claim is mode B only. B does **not** turn Keep the watch off and does **not** put the Mac to sleep by itself. Keep the watch still holds closed-lid wake. Agents keep running while B has the panel asleep. Never invent display sleep while the lid is open.
...
| Lid-close hygiene | On **confirmed** lid **close** (not on toggle, not a single raw `AppleClamshellState` edge): turn **keyboard backlight off**. **Power A (default, landed, Mac-proven):** set brightness to the **user floor %** (default **15%**, range 1–40; never 0%). Brightness write only — not display sleep, not “screen off”, not `displaysleepnow`. Floor write only while armed + lid confirmed closed + Power A. **Power B (landed, Mac-proven):** `displaysleepnow` (or equivalent — Core picks the smallest reliable path) instead of the floor. Mutually exclusive with A — never floor + `displaysleepnow` together. This **is** display sleep for B only — **panel/display only**. B does **not** turn Keep the watch off and does **not** put the Mac to sleep by itself. Keep the watch still holds closed-lid wake (IOPM / SleepDisabled as already specified). Agents keep running while B has the panel asleep. Unconfirmed close applies neither. Universal — no M2-only branches. |
...
- Card map:
  ...
  - **Power:** mutually exclusive panel mode A/B (landed, Mac-proven; default A; Dim panel / Sleep panel picker + caption + help that teach A vs B in the popover) + brightness floor % + keyboard backlight off, battery auto-off, brightness return ramp (visible in A; hide while B), thermal auto-off toggle (default ON; unlocked)
  - **Agents:** idle wait after local busy signals stop + per-tool include multi-select (landed, Mac-proven)
  - **Notif:** landed outbound POST (Discord URL + Telegram token + chat id; dotted + reveal; clear secrets); landed Telegram inbound on/off; landed slash `/arm` `/disarm` `/status` `/help`, wake-miss drain, lid-gated Telegram `/disarm` (Mac-proven); facts-richer `/status` dump + `/help` mirror (Mac-proven, reply copy); live battery on `/status` (Mac-proven, reply copy); Discord inbound **landed** (Core + UI; bot token + channel id; webhook stays outbound-only — not Mac-proven until soft Mac optical)
...
- **Honesty:** Mac optical on smooth section-switch **passed** (Watch ↔ Power ↔ Agents ↔ Notif ↔ General — no ghost paint, height hugging). Open-lid brightness and Agents picker / OpenCode process/session **passed** (user optical, 2026-09-24). Power A/B (Dim panel / Sleep panel; A↔B exclusive + height) and Telegram `/status` live battery **passed** (user optical, 2026-09-25) — do not claim Discord inbound Mac-proven from this check. Still not every provider. Still not think-detection.
...
- Brightness return when the lid opens — Core + popover control; **1 / 2 / 3 s**, default **2s**. **Power A:** restore captured brightness only; nil capture skips the write (do not fall back to floor). **Power B:** hide this ramp control/chrome while B is selected (ramp only applies when restoring from the floor path). Power help must say why it is hidden.
...
- **Notif** — V2 slice, popover-only. Outbound idle-after-wait POST is **landed**. Telegram inbound two-way is **landed** (Mac-proven). Telegram inbound polish is **landed** (Mac-proven): BotFather slash menu `/arm` `/disarm` `/status` `/help`, wake-miss drain, lid-gated Telegram `/disarm`. Facts-richer Telegram `/status` dump + `/help` mirror is **landed** (Mac-proven, user optical 2026-09-24): dump live WatchEngine state Agrypnos already knows. Live laptop battery on `/status` is **landed** (Mac-proven, user optical 2026-09-25) — same dump; reply copy only, not a Notif card. Discord inbound is **landed** (Core + UI — user’s own Discord bot token + channel id; webhook stays outbound-only; same WatchEngine path as Telegram). Not Mac-proven until soft Mac optical. Do not grow past that. Enriched-status B (task text / finish ETA) is **scrapped** — will not implement; Review blocks any PR that adds it.
  ...
  - **Telegram `/status` facts dump (landed, Mac-proven — user optical 2026-09-24).** Dump live WatchEngine state Agrypnos already knows. Facts only. Omit lines that don’t apply or aren’t known. Enriched-status B (task text / finish ETA) is **scrapped** — will not implement; Review blocks any PR that adds it. Ban invent ETA, “still thinking,” “job finished,” think-detection, and a countdown on remembered-only How long (`∞` / `1h` / `3h` / custom do not auto-off; Agents settle is not that clock). `/status` reply includes:
    ...
    6. Safety facts that affect the watch: low-battery threshold; thermal auto-off on/off. LPM copy honesty as already specified (no ended-copy while forced watch still holds) — omit an LPM line if that state is unknown; do not invent a sensor. **Landed (Mac-proven, user optical 2026-09-25):** live laptop battery — charge % and discharging vs on AC from the same sensor Core already uses for low-battery auto-off. Shape: `Battery 62% · discharging` or `Battery 62% · on AC` (plain equivalent OK). Omit if unknown. Keep the threshold + thermal lines. Reply copy only — not a Notif battery gauge card. Ban time-to-empty, health-gauge / °C / warranty, invent %.
    Soft verify of the dump + `/help` mirror: **passed** (user optical, 2026-09-24). Soft Mac prove of live battery: **passed** (user optical, 2026-09-25 — live % matches the Mac menu bar, or omit if unknown; `/help` short mention that `/status` includes live battery when known).
  - **Channels:** (1) Discord **incoming webhook URL** the user creates in their own server — outbound POST only; never inbound. (2) Telegram **user’s own bot** — token + chat id from BotFather / `getUpdates` for outbound; same bot for inbound commands. (3) Discord **user’s own bot** — bot token + **channel id** allowlist for inbound (not the webhook). **No shared Agrypnos bot. No companion app. No telemetry / stealth network.**
  - **Secrets** (webhook URL / Telegram bot token / chat id / Discord bot token / Discord channel id) live in Application Support (`notif-secrets.json`, mode 0600). **Not Keychain** — unsigned builds prompt for the login password, which looks like Agrypnos wants the Mac password. Never plaintext prefs, logs, or README examples with real secrets. Popover fields are dotted; an eye button reveals the value.
  - **UI:** **Notif** after Agents: **Watch · Power · Agents · Notif · General**. Default still **Watch**. Controls: outbound opt-in (default OFF); Discord URL field (webhook — outbound-only); Telegram token + chat id fields; reveal buttons on those fields; clear/remove that deletes the saved file; **Telegram inbound on/off + short command list / help** (landed; polish landed: slash `/arm` `/disarm` `/status` `/help`, wake-miss, lid-gated `/disarm`; Mac-proven). **Discord inbound on/off + short command/help list + bot token + channel id** (landed — Core + UI; chrome mirrors Telegram; keep the webhook paste block outbound-only; never imply the webhook receives commands; not Mac-proven until soft Mac optical). Facts-richer `/status` is reply copy (**landed**, Mac-proven), not a new Notif control. Live battery on `/status` is **landed** (Mac-proven, same reply copy, not a Notif card). Still **no** separate settings window. Still no Licence/About tabs. Still no fake remote-control panel.
  - **Plain copy:** “POST to *your* webhook” / “message *your* Telegram bot” / “Commands on *your* Telegram bot: /arm, /disarm, /status, /help.” / “Commands on *your* Discord bot: /arm, /disarm, /status, /help.” / “The Discord webhook is outbound-only. It does not receive commands.” Ban “we notify your phone”, “agent stopped”, “job finished”, live-asleep replies, “disarm always sleeps”.
  - **Honesty:** the user owns the webhook/bot. Agrypnos only POSTs when outbound Notif is enabled and the matching secrets are set. Agrypnos only accepts Telegram commands when inbound is on and token + chat id are set, and only while it can poll (Mac awake). Agrypnos only accepts Discord commands when Discord inbound is on and bot token + channel id are set, and only while it can receive updates (Mac awake). No live-asleep reply — no relay. Discord webhook is outbound-only.
  - **Docs:** `SECURITY.md` allows opt-in POST to the user’s webhook/bot and inbound poll/update on that same user Telegram bot and receive-on-the-Mac updates on the user’s Discord bot; still no telemetry / stealth network. Do not write “no network calls” as an absolute. Still no shared bot. Name that confirmed lid-closed inbound `/disarm` (Telegram landed; Discord landed — Core + UI, not Mac-proven) may `pmset sleepnow` (not a new sudoers grant). Name that Discord inbound must not set an Interactions Endpoint URL or open a listen port.
  - **Self-serve docs (required):**
    - README: step-by-step Discord **outbound** (Server Settings → Integrations → Webhooks → New Webhook → copy URL) **separate from** Discord **inbound** (Developer Portal → New Application → Bot → Reset Token; invite with `bot` + `applications.commands`; Developer Mode → Copy Channel ID; paste token + channel id in Discord inbound fields — not the webhook URL). Telegram (BotFather `/newbot` → token; message the bot; get chat id via `getUpdates` or a clear equivalent). What to paste where in Agrypnos. How to test outbound (arm Agents, produce a local busy signal, wait idle, expect one POST, then Keep the watch off). How to enable Telegram inbound (same BotFather bot; chat with the bot; inbound on in Notif; `/arm` `/disarm` `/status` `/help`). How to enable Discord inbound (own bot token + channel id; inbound on in Notif; same four commands on the shared WatchEngine path). BotFather / Discord `/` menu matches those four via `setMyCommands` or Discord application commands (or equivalent); `/help` asleep note + confirmed lid-gated `/disarm`; `/help` says `/status` returns these watch facts (short) — dump is **landed** (Mac-proven, user optical 2026-09-24); `/help` also says `/status` includes live battery when known (Mac-proven, user optical 2026-09-25). Wake-miss drain without applying (not leftover setup/quit drain). How to turn off / clear secrets. Do not describe Discord inbound as Mac-proven until soft Mac optical.
    - In-app Notif help must point at the same steps (short in the popover; full detail in README, linked or paraphrased).
  - **Out of this slice:** Enriched-status B (task text / finish ETA) is **scrapped** — will not implement; Review blocks any PR that adds it. Live battery on `/status` is **landed above** (Mac-proven), not that scrapped item. Discord inbound is **landed below** (Core + UI; webhook stays outbound-only; bot-token inbound — not Mac-proven until soft Mac optical). Notification Center as the Notif path stays idea-only. Do not invent them. Power A/B is **landed** in Power (Mac-proven, not this Notif slice).
...
**Landed (Mac-proven):** live laptop battery on Telegram `/status` — extend the landed dump. Live charge from the same sensor Core already uses for low-battery auto-off. Shape: `Battery 62% · discharging` or `Battery 62% · on AC` (plain equivalent OK). Omit if unknown. Keep low-battery threshold + thermal on/off lines. Reply copy only — not a Notif battery gauge card. `/help` says (short) that `/status` includes live battery when known; keep asleep note + lid-gated `/disarm` + the existing watch-facts line. User optical **passed** 2026-09-25 (live % matches the Mac menu bar, or omit if unknown). Ban time-to-empty, health-gauge / °C / warranty, invent %. Facts-richer dump stays landed/Mac-proven. Enriched-status B (task text / finish ETA) is **scrapped**.

**Landed (Core + UI; not Mac-proven until soft Mac optical):** Discord inbound two-way — user’s own Discord bot.
- **Same WatchEngine path.** Mirror Telegram’s `/arm` `/disarm` `/status` `/help` (Discord-native slash may map to those same commands). Register Discord application commands (or equivalent) so the `/` menu matches. No second arm/disarm stack.
- **Webhook stays outbound-only.** Never mix inbound with the incoming webhook URL. Inbound needs a separate **bot token** + allowlist. Receive on the Mac (Gateway or equivalent — Core picks the smallest reliable path). Do **not** set Discord’s Interactions Endpoint URL. Do **not** open a listen port. The interactions URL is not a substitute inbound.
- **Allowlist:** saved **channel id** (same role as Telegram chat id). Commands only from that channel. Empty token or empty channel id: no inbound. Default inbound **off**, separate from the outbound POST opt-in.
- **No shared Agrypnos Discord bot.** User owns bot + webhook.
- **Honesty:** facts-only replies; ban “agent stopped / job finished / still thinking”; no live-asleep reply / no relay; wake-miss drain without apply + missed-while-asleep (same rules as Telegram — drain **if** anything was queued; do not scrape channel history on wake to invent missed commands). Lid-gated Discord `/disarm` same as Telegram (lid open or unconfirmed: clear hold, no sleep; confirmed closed: clear hold + `pmset sleepnow`). `/status` dumps live WatchEngine (including live battery when known). `/help` keeps the asleep note (if the bot isn’t replying, the Mac is likely asleep / Agrypnos isn’t receiving updates) + lid-gated `/disarm` + the watch-facts line. Enriched-status B stays **scrapped**.
- **Secrets:** Application Support `notif-secrets.json` mode 0600, dotted + reveal — same as existing Notif secrets. Not Keychain.
- **UI (landed on main; not Mac-proven):** Notif Discord inbound chrome mirrors Telegram (inbound on/off + short command/help list + bot token + channel id). Keep the webhook paste block outbound-only — never imply the webhook receives commands.
- Soft Mac optical (still open): inbound on → `/` menu shows the four commands; `/help` honesty + asleep note + lid-gated `/disarm`; cmds while asleep → on wake missed-while-asleep, no auto-apply; lid open `/disarm` → watch off, Mac stays awake; confirmed lid-closed `/disarm` → watch off + Mac sleeps. Webhook stays outbound-only. No live-asleep reply. Do not claim Mac-proven until that check. Core + UI landed; soft Mac still open.

**Landed (Mac-proven):** Power A/B — mutually exclusive panel modes. Default **A**. Switching A↔B must be clear in **Power** (segmented or equivalent native control). Same gates as hygiene: armed + lid confirmed closed (`LidCloseConfirm` / stable closed — not a single raw `AppleClamshellState` edge). Unconfirmed close must not apply floor or `displaysleepnow`. Keyboard backlight off + battery / thermal / LPM safety still apply under both. Never floor + `displaysleepnow` together.
...
- **B:** real **panel/display** sleep on confirmed lid-close via `displaysleepnow` (or equivalent — Core picks the smallest reliable path). Hide the lid-open ramp control/chrome while B is selected (ramp only applies when restoring from the floor path). This **is** display sleep for mode B only — do not claim it for A or for the Keep the watch toggle. B sleeps the **panel/display only**. It does **not** turn Keep the watch off and does **not** put the Mac to sleep by itself. Keep the watch still holds closed-lid wake (IOPM / SleepDisabled as already specified). Agents keep running while B has the panel asleep. Plain copy: “Display asleep.” / “Panel sleeps on confirmed lid close. Keep the watch still holds the Mac awake.” Ban for Power B: “Mac asleep,” “agents stopped,” “job finished,” “still thinking,” “puts the computer to sleep,” or any wording that implies B ends Keep the watch or ends agent work.
- **In-app explanation (required — popover Power, not README-only).** Review blocks a Power A/B control with no in-app explanation of the difference. Ban watt claims. Ban marketing fluff.
  - **Picker labels:** short names (e.g. “Dim panel” / “Sleep panel” — or clearer plain equivalents UI may craft).
  - **Caption under the picker:** always visible when Power is open. One or two plain sentences that state the difference. **A (default):** brightness floor on confirmed lid-close + lid-open ramp. Panel stays on, dimmed. Do **not** say display asleep / screen off. **B:** on confirmed lid-close, panel sleeps (`displaysleepnow`). Keep the watch still holds the Mac awake — agents keep running. Ban “Mac asleep” / “agents stopped.”
  - **Help:** Power help / detail text — same honesty, slightly longer OK. When B is selected, hide ramp chrome and say why (ramp only applies when restoring from the floor path).
Soft Mac prove **passed** (user optical, 2026-09-25): Dim panel / Sleep panel picker + caption; A = floor+ramp, panel stays on, no surprise display-sleep claim; B = confirmed lid-close panel actually sleeps, ramp hides, help says why, Keep the watch still on, Mac not put to sleep by B, agents still running; A↔B exclusive + height (never floor + `displaysleepnow` together); open-lid disarm/use does not leave the panel wrongly asleep. Unconfirmed close applies neither. Ban watt claims, Wi-Fi/BT kill, inventing sleep while the lid is open. Enriched-status B (task text / finish ETA) is **scrapped**.

**Landed (Mac-proven):** facts-richer Telegram `/status` dump + `/help` mirror — dump live WatchEngine state Agrypnos already knows (Keep the watch on/off; How long mode named, no fake countdown on remembered-only; lid open or unconfirmed vs confirmed closed; Agents selected / busy-seen-this-arm / settle-wait when relevant; last-end honesty if any; safety prefs that affect the watch). Reply copy only — not a new Notif popover control. `/help` says (short) that `/status` returns these watch facts; keep asleep note + lid-gated `/disarm`. User optical **passed** 2026-09-24. Ban invent ETA, think-detection, countdown on remembered-only How long. Live battery is **landed above** (Mac-proven, user optical 2026-09-25). Enriched-status B (task text / finish ETA) is **scrapped**.

**Landed (Mac-proven):** Telegram inbound polish — BotFather slash menu `/arm` `/disarm` `/status` `/help` (`setMyCommands` or equivalent); wake-miss drain (no live-asleep reply; drain queued commands without applying; facts-only missed-while-asleep); lid-gated force-sleep on Telegram `/disarm` only (lid open or unconfirmed: clear hold, no sleep; confirmed lid-close: clear hold and `pmset sleepnow`). User optical **passed** 2026-09-24 (all five).

**Landed (Mac-proven):** Open-lid brightness — no surprise dim to floor after disengage with the lid open (user optical, 2026-09-24). Floor write only when armed + lid confirmed closed + Power A; nil capture skips the write. Power B must not leave the panel wrongly asleep on open-lid disarm/use (**passed**, user optical 2026-09-25).
...
Notification Center as the Notif path stays idea-only. Do not unlock.
...
App Store sandbox, notarization pipeline, providers beyond Cursor, Claude Code, Codex, and OpenCode, fake benchmarks, Wi-Fi/BT kill, Dock UI, separate settings window, **Licence** tab, **About** as a toolbar tab, donate without a live URL, `displaysleepnow` on engage / while the lid is open, claiming display sleep or “screen off” for Power A or for the Keep the watch toggle, claiming Power B puts the Mac to sleep / turns Keep the watch off / ends agent work, think-detection / “still thinking” claims, mixing Discord inbound with the incoming webhook (webhook stays outbound-only), Discord Interactions Endpoint URL / listen-port inbound, enriched-status B (task text / finish ETA — **scrapped**, will not implement), a shared Agrypnos bot, Notification Center as the Notif path, inventing ETA in Telegram or Discord replies, live-asleep Telegram or Discord replies / a sleep relay, applying Telegram or Discord commands that arrived while the Mac was asleep, “disarm always sleeps.”
...
- Soft verify (live battery on Telegram `/status`): **passed** (user optical, 2026-09-25) — live % matches the Mac menu bar (`Battery 62% · discharging` / `Battery 62% · on AC` or plain equivalent; omit if unknown). Keep threshold + thermal lines. `/help` short mention that `/status` includes live battery when known. Ban time-to-empty, health-gauge / °C / warranty, invent %.
- Soft verify (Power A/B): **passed** (user optical, 2026-09-25) — Dim panel / Sleep panel picker + caption; A = confirmed lid-close floor + ramp, panel stays on, no surprise display-sleep claim; B = confirmed lid-close **panel** actually sleeps (`displaysleepnow` or equivalent); ramp chrome hidden; help says why; Keep the watch still on; Mac not put to sleep by B; agents still running; A↔B exclusive + height (never floor + `displaysleepnow` together). Open-lid disarm/use does not leave the panel wrongly asleep. Unconfirmed close applies neither. Copy must not say B made the Mac asleep, ended Keep the watch, or ended agent work. Ban watt claims.
- Soft verify (Discord inbound): soft Mac still open. Inbound on → Discord `/` menu shows `/arm` `/disarm` `/status` `/help` (or Discord-native slash mapped to those); `/help` honesty + asleep note + lid-gated `/disarm`; cmds while asleep → on wake “missed while asleep”, no auto-apply; lid open `/disarm` → watch off, Mac stays awake; confirmed lid-closed `/disarm` → watch off + Mac sleeps. Same WatchEngine path as Telegram — no second arm/disarm stack. Webhook still outbound-only. No live-asleep reply. Core + UI landed. Do not claim Mac-proven until that check.
...
| **Review** | Gates. File size, TDD, no watt fiction, no god files, no false display-sleep claims for Power A or the Keep the watch toggle, floor write only on armed + confirmed lid close + Power A (not a single raw `AppleClamshellState` edge; never floor under Power B), nil display capture never writes floor, no surprise dim after disengage with the lid open, no M2-only lid/hygiene branches, open-lid brightness is Mac-proven (user optical 2026-09-24), plain popover copy, no false ended-copy under LPM forced-watch, no settings window, no Licence/About tabs, Notif is landed V2 (popover switcher **Watch · Power · Agents · Notif · General**, one-way idle-after-wait, Application Support secrets `0600` + dotted/reveal, self-serve Discord/Telegram, no shared bot, no Keychain login prompt; Telegram two-way is landed and Mac-proven — user’s bot, inbound on/off + command help in Notif, replies are facts only; Telegram inbound polish is landed and Mac-proven — slash `/arm` `/disarm` `/status` `/help` + BotFather `setMyCommands` (or equivalent) so the menu matches; `/help` asleep honesty (if the bot isn’t replying, Mac likely asleep / Agrypnos isn’t polling); no live-asleep reply / no relay; wake-miss drain without applying (auto-apply on wake is a regression) then facts-only missed-while-asleep; Telegram `/disarm` lid-gated only (lid open or unconfirmed: clear hold, never sleep; confirmed lid-close: clear hold and `pmset sleepnow`); do not ship disarm-always-sleeps; popover/hotkey disarm stay as specified (sleep path is inbound `/disarm` only — Telegram landed; Discord landed (Core + UI, not Mac-proven), same WatchEngine path); facts-richer Telegram `/status` dump + `/help` mirror is landed and Mac-proven (user optical 2026-09-24; reply copy, not a new Notif control; `/help` names these watch facts and keeps asleep note + lid-gated `/disarm`); live laptop battery on `/status` is landed and Mac-proven (user optical 2026-09-25 — same sensor as low-battery auto-off; shape `Battery 62% · discharging` / `Battery 62% · on AC` (omit if unknown); keep threshold + thermal lines; reply copy only, not a Notif battery gauge card; `/help` short mention that `/status` includes live battery when known); Power A/B is landed and Mac-proven (user optical 2026-09-25 — mutually exclusive; default A (floor+ramp, panel stays on dimmed, ban display-sleep copy for A); B is real **panel/display** sleep on confirmed lid-close via `displaysleepnow` (or equivalent) only — does not turn Keep the watch off, does not put the Mac to sleep by itself; Keep the watch still holds closed-lid wake; agents keep running while B has the panel asleep; hide ramp chrome while B; never floor+`displaysleepnow` together; same armed + lid-confirm gates; keyboard off + battery/thermal/LPM still apply; ban Power B copy “Mac asleep,” “agents stopped,” “job finished,” “still thinking,” “puts the computer to sleep”; in-app Power explanation required (picker labels + caption always visible when Power is open that states A vs B; help when B hides ramp chrome and says why); Review blocks a Power A/B control with no in-app explanation of the difference; ban watt claims; still block invent ETA, think-detection, countdown on remembered-only How long, time-to-empty, health-gauge / °C / warranty, invent battery %, `displaysleepnow` on toggle or while the lid is open, watt claims, Wi-Fi/BT kill, shared bot, Discord-webhook-as-inbound (webhook never receives commands), Discord inbound landed (Core + UI; not Mac-proven until soft Mac optical — user’s own bot token + channel id allowlist; default off; empty token/channel id = no inbound; no shared Discord bot; same WatchEngine path as Telegram `/arm` `/disarm` `/status` `/help`; receive on the Mac — Review blocks Interactions Endpoint URL / listen-port inbound; same wake-miss + lid-gated `/disarm` + facts-only `/status` including live battery when known; secrets `0600` dotted/reveal not Keychain; Notif chrome mirrors Telegram; Review blocks a second arm/disarm stack), NC-as-Notif; enriched-status B (task text / finish ETA) is **scrapped** — Review blocks any PR that adds it), status item is landed (**Armed.** / **Agents.** non-countdown; no fake timer), per-tool Agents include is landed and Mac-proven (multi-select Cursor · Claude Code · Codex · OpenCode; require ≥1; default all on; no separate track-all toggle; busy signals only from selected tools; OpenCode is local process + session files, Mac-proven; still not every provider, still not think-detection), section-switch popover height must animate (block sudden resize / hard cut; no cross-section paint; height hugs the active section), donate gated on live URL | Shipping unreviewed slop |
| **Boss** | Sequence, merge order, “stop” on parked V2 (Notification Center as Notif); Power A/B + live battery on `/status` are landed / Mac-proven (user optical 2026-09-25); Discord inbound is landed (Core + UI; not Mac-proven until soft Mac optical); block scrapped B (task text / finish ETA) | Writing all the code |
...
**Hard:** every popover caption says what the control does. Ban mysterious metaphors for real behavior — no “kill the keys,” “floor the panel,” “sleeps with you,” or vague “when they settle” / “go quiet” as the only explanation. (Internal/product terms like *settle grace* in this file are fine; idle-wait **title** may stay short; **help** must say local busy signals → settle buffer → idle-after-wait POST and Keep the watch off. Ban “still thinking” / “agent finished.” Power B may say the panel sleeps with confirmed lid close and that Keep the watch still holds the Mac awake — that is the honest B caption, not the “sleeps with you” metaphor. Ban Power B copy that says “Mac asleep,” “agents stopped,” “job finished,” “still thinking,” “puts the computer to sleep,” or that B ends Keep the watch or ends agent work. Power A/B must teach the difference **in the popover** — picker labels + a caption always visible when Power is open. Ban a naked A/B control with no in-app explanation.)

Good: “Armed. Waiting for lid close — then brightness floor + keyboard backlight off. Auto-off at 15% battery.” (Power A)
Good: “Armed. Waiting for lid close — then the panel sleeps + keyboard backlight off. Keep the watch still holds the Mac awake.” (Power B)
Good: “Brightness floor. Panel stays on (dimmed).” (Power A)
Good: “Display asleep.” (Power B)
Good: “Panel sleeps on confirmed lid close. Keep the watch still holds the Mac awake.” (Power B)
...
Good: “Commands on *your* Discord bot: /arm, /disarm, /status, /help.”
Good: “The Discord webhook is outbound-only. It does not receive commands.”
Good: “If the bot isn’t replying, the Mac is likely asleep / Agrypnos isn’t polling.”
Good: “If the bot isn’t replying, the Mac is likely asleep / Agrypnos isn’t receiving updates.” (Discord `/help` — landed; not Mac-proven until soft Mac optical)
...
Good: “Armed.” / “Disarmed.” / “Keep the watch is on.” (Telegram or Discord arm/disarm replies — facts only)
Good: “Keep the watch is on. How long is Agents. Lid open.” (Telegram or Discord `/status` — facts only; omit lines that don’t apply)
Good: “Battery 62% · discharging” / “Battery 62% · on AC” (Telegram `/status` live battery, Mac-proven; Discord `/status` after inbound ships — omit if unknown)
...
Good: “/status includes live battery when known.” (`/help` — short; live battery Mac-proven)
Good: “Dim panel” / “Sleep panel” (Power A/B picker — landed; or clearer plain equivalents)
Good: “Dim panel: brightness floor on confirmed lid close. Panel stays on, dimmed. Sleep panel: the panel sleeps. Keep the watch still holds the Mac awake — agents keep running.” (Power caption under the picker — landed)
Good: “Brightness return is hidden because it only applies when restoring from the floor path.” (Power help while B — landed)
...
Bad: a Power A/B picker with no caption / no in-app explanation of the difference.
Bad: watt claims or marketing fluff on the Power A/B picker (“smart sleep,” “maximum battery”).
Bad: “We track every AI.”
Bad: “1h 12m remaining” on the status item or Telegram / Discord `/status` when How long is remembered-only and will not auto-off.
...
Bad: “Mac asleep.” / “Agents stopped.” / “Puts the computer to sleep.” for Power B (B sleeps the panel/display only; Keep the watch still holds; agents keep running).
Bad: ended/standing-down copy while Low Power Mode forced-watch is still holding the Mac awake.
Bad: “We notify your phone.” / “Agent stopped.” / “Job finished.” (Notif outbound is idle after wait; POST to *your* webhook / message *your* Telegram bot. Telegram or Discord `/status` dumps live watch facts, not that the agent stopped. Arm/disarm replies stay armed/disarmed facts — same ban.)
...
Bad: applying a Telegram or Discord command that arrived while the Mac was asleep as if it ran live.
Bad: treating the Discord webhook as inbound / implying it receives commands.
Bad: setting Discord’s Interactions Endpoint URL, opening a listen port, or using a public callback as Discord inbound.
