...
- Card map:
  ...
  - **Notif:** landed outbound POST (Discord URL + Telegram token + chat id; dotted + reveal; clear secrets); landed Telegram inbound on/off; unlocked slash `/arm` `/disarm` `/status` `/help`, wake-miss drain, lid-gated Telegram `/disarm`
...
- **Notif** — V2 slice, popover-only. Outbound idle-after-wait POST is **landed**. Telegram inbound two-way is **landed**. Telegram inbound polish is **unlocked** (Core + UI after Review CLEAR + Boss merge of this bar): BotFather slash menu `/arm` `/disarm` `/status` `/help`, wake-miss drain, lid-gated Telegram `/disarm`. Do not grow past that.
  ...
  - **Telegram inbound two-way (landed).** User’s own bot — **same token** already required for Telegram outbound. Accept commands only from the saved chat id. Empty token or chat id: no inbound. Default inbound **off** (separate from the outbound POST opt-in). Not a shared Agrypnos bot. Not Discord inbound (webhook stays outbound; Discord bot-token inbound stays idea-only / separate). Command replies are plain armed / disarmed / status facts. Ban “agent stopped / job finished / still thinking.” No fake remote-control panel. **Honesty:** Mac prove of Telegram two-way remains **soft** — do not claim Mac-proven until that Mac check. Soft prove of the polish below stays **soft** after implement.
  - **Telegram inbound polish (unlocked — Core + UI after Review CLEAR + Boss merge of this bar).** Same user’s own bot; no shared Agrypnos bot. Product command list is slash forms **`/arm`** **`/disarm`** **`/status`** plus **`/help`** (facts only) → real WatchEngine arm/disarm/status (`/help` is copy only). Register BotFather **`setMyCommands`** (or equivalent) so the bot menu matches those four. Do not invent ETA in replies.
    - **Wake-miss drain.** No live “Mac asleep” reply while the Mac is sleeping — no relay. `/help` must say: if the bot isn’t replying, the Mac is likely asleep / Agrypnos isn’t polling. On wake: commands that arrived while asleep must be **drained without applying** (today’s auto-apply on wake is a bug vs this bar); reply facts-only **missed while asleep** (or equivalent plain wording). Ban copy that implies Agrypnos answered during sleep. Leftover drain on poller start (setup / quit / inbound off) stays ack-without-apply and is not this wake-miss reply.
    - **Lid-gated Telegram `/disarm` only.** If the Mac is awake and the **lid is open**: turn Keep the watch off / clear wake hold, **do not** send the Mac to sleep. If the Mac is awake and the **lid is closed**: turn Keep the watch off / clear wake hold **and** send the Mac to sleep (`pmset sleepnow` or equivalent — Core picks the smallest reliable path). Explain this in `/help`. Lid-open `/disarm` must never sleep the Mac. Do not ship “disarm always sleeps.” Popover and hotkey disarm stay as already specified — this sleep path is Telegram `/disarm` only.
  ...
  - **Docs:** `SECURITY.md` allows opt-in POST to the user’s webhook/bot and inbound poll/update on that same user bot; still no telemetry / stealth network. Do not write “no network calls” as an absolute. Still no shared bot. Name that lid-closed Telegram `/disarm` may `pmset sleepnow` (not a new sudoers grant).
  - **Self-serve docs (required):**
    - README: step-by-step Discord (Server Settings → Integrations → Webhooks → New Webhook → copy URL) and Telegram (BotFather `/newbot` → token; message the bot; get chat id via `getUpdates` or a clear equivalent). What to paste where in Agrypnos. How to test outbound (arm Agents, produce a local busy signal, wait idle, expect one POST, then Keep the watch off). How to enable inbound (same BotFather bot; chat with the bot; inbound on in Notif; BotFather menu `/arm` `/disarm` `/status` `/help` via `setMyCommands` or equivalent). `/help` asleep note + lid-gated `/disarm`. Wake-miss: drain without applying; reply missed while asleep. How to turn off / clear secrets. Discord inbound is not this slice.
...
**Unlocked (implement next, Core + Agrypnos UI):** Telegram inbound polish — BotFather slash menu `/arm` `/disarm` `/status` `/help` (`setMyCommands` or equivalent); wake-miss drain (no live-asleep reply; drain queued commands without applying; facts-only missed-while-asleep); lid-gated force-sleep on Telegram `/disarm` only (lid open: clear hold, no sleep; lid closed: clear hold and `pmset sleepnow`). After Review CLEAR + Boss merge of this bar. Mac prove remains **soft**. Inbound two-way itself is landed. Discord inbound stays parked.
...
- Soft verify (Telegram two-way): after implement, `/arm` `/disarm` `/status` `/help` on the user’s Telegram bot (BotFather slash menu matches). `/help` includes the asleep note and lid-gated `/disarm`. Wake-miss: commands queued while the Mac is asleep are drained without applying; facts-only missed-while-asleep reply. Lid-closed Telegram `/disarm` → Mac sleeps; lid-open `/disarm` → Keep the watch off, no sleep. Do not claim Mac-proven until that check. No live-asleep reply.
...
Good: “/disarm with the lid open turns Keep the watch off and does not sleep the Mac. With the lid closed it turns Keep the watch off and sends the Mac to sleep.”
