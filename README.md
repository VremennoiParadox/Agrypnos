...
V1 agents: Cursor, Claude Code, Codex, and OpenCode. Local heuristics only (process list + session-file mtimes; Claude/Codex/OpenCode may also use CPU). OpenCode looks at `~/.local/share/opencode` (or `$XDG_DATA_HOME/opencode`) — prove that process + session mtime on a Mac before claiming detection. Which tools count as busy is a multi-select of those four (at least one stays on; default all on). Core remembers that set; the Agents picker chrome is not in the current app yet, so all four count until it lands. Correctness over coverage. Not every provider.
...
3. Run an agent Agrypnos can see so a **local busy signal** is recorded this arm. Cursor, Claude Code, Codex, and OpenCode all count by default. OpenCode’s local process + session-file paths still need a Mac prove. The Agents picker chrome is not in the current app yet — until it lands, you cannot deselect a tool.
