...
The menu-bar extra shows **Armed.** while Keep the watch is on (∞ / 1h / 3h / custom minutes) and **Agents.** when How long is Agents. Off is the glyph only. Not a remaining-time countdown — How long timers do not auto-off. Still locked: per-tool Agents include list. Donate stays gated until there is a live URL.
