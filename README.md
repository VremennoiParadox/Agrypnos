...
V1 agents: Cursor, Claude Code, Codex, and OpenCode. Local heuristics only (process list + session-file mtimes; Claude/Codex may also use CPU). OpenCode is local process + session files — prove that on a Mac before claiming detection. Which tools count as busy is a multi-select of those four (at least one stays on; default all on). That multi-select is the next Agents control; it is not in the current app yet. Correctness over coverage. Not every provider.
...
3. Run an agent Agrypnos can see so a **local busy signal** is recorded this arm. In the current app that is Cursor, Claude Code, or Codex. OpenCode and the Agents multi-select are next: once they land, only tools you left on count, and OpenCode counts after its local process + session-file heuristic is proven on a Mac.
