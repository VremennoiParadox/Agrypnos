...
- Armed ≠ black screen. Power A (default, shipped) floors brightness on confirmed lid close — that is not display sleep. Power B (real panel sleep on confirmed lid close) is the **next slice** — not claimed shipped. The Keep the watch toggle never sleeps the panel.
