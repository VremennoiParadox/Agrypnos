...
- Telegram two-way (arm / disarm / status on *your* bot) is in Notif: inbound on/off (default **off**, separate from outbound POST) plus the short command list. Same token + chat id as outbound. Do not claim Mac-proven until a Mac checks arm / disarm / status.
...
- **Notif:** opt-in idle-after-wait POST (default **off**). Opt-in switch, Discord URL, Telegram token + chat id, Telegram inbound on/off (default **off**, separate from the POST opt-in) with commands arm / disarm / status on *your* bot, and Clear secrets. Self-serve setup lives in the Notif section; same steps: [Notif](#notif).
...
Core polls `getUpdates` and drives Keep the watch when inbound is on and token + chat id are set. Turn **Telegram inbound** on in **Notif** (default **off**, separate from the outbound POST opt-in). Each time the poller starts, the first `getUpdates` is immediate (timeout 0) and acks already-queued messages without running them, so leftover traffic (setup, app quit, inbound off) does not arm the watch. After that, Agrypnos long-polls for new commands. Do not claim Mac-proven from this README.
