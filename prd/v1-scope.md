...
A human closes a MacBook lid overnight while Cursor / Claude Code / Codex work. The machine must stay up. The display must actually sleep. The keyboard must go dark. When the agents settle, sleep is allowed again. Safety auto-offs exist so the notebook is not cooked or drained.
...
5. Auto-off: timer end, battery floor while discharging, thermal serious/critical, Low Power Mode on battery (hard battery floor always wins).
6. Agents mode: busy holds awake; after busy, idle for grace allows sleep. Cursor + Claude Code + Codex only. Process + session mtime. (Core tests.)
